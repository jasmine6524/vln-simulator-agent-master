...
def render_video(meshes, angle, key, action, renderer, savepath, background, cam=(0.75, 0.75, 0, 0.10), color=[0.11, 0.53, 0.8]):
    writer = imageio.get_writer(savepath, fps=30)
    # center the first frame
    meshes = meshes - meshes[0].mean(axis=0)
    theta_angle = (np.pi / 180 * float(angle))
...
def main():
    ...
    parser.add_argument('-f','--filename',default= None,help='input file location')
    parser.add_argument('-a','--angle',default= 16,help='angle of axis, result equals pi / 180 * angle')
    ...
    for key in output:
        ...
        for action in range(len(vidmeshes)):
            ...
            render_video(meshes, angle, key, action, renderer, path, background)
